...
const double _kTwoPi = math.pi * 2;
...
const double _kTimePickerHeightPortrait = 456.0;
...
class _HourMinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const style = TextStyle(
      color: Colors.white,
      // backgroundColor: Color(0xff6d9f42),
      fontSize: 35,
...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final radius = size.shortestSide / 1.7;
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  void _handleTapUp(TapUpDetails details) {
    ...
    _animateTo(
      _getThetaForTime(
        _getTimeForTheta(_theta.value, roundMinutes: true),
      ),
    );
    _dragging = false;
    _position = null;
  ...
  _TappableLabel _buildTappableLabel(
    ...
    final style = textTheme.bodyText1!.copyWith(color: color);
...
Future<TimeOfDay?> showMyMaterialTimePicker({
